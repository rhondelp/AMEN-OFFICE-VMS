...
$id = intval($_GET['id'] ?? 0);
$data = query_single("SELECT * FROM visitors WHERE id = ?", [$id], 'i');
...
<div class="badge-card bg-white w-[380px] rounded-3xl shadow-xl shadow-surface-900/5 overflow-hidden border border-surface-200/60">
    ...
    <div class="px-8 pt-8 pb-7 text-center">
        <div class="flex items-center justify-center gap-2.5 mb-6">
            ...
            <span class="text-[10px] font-semibold text-surface-800/25 uppercase tracking-[0.15em]">Amen's Office</span>
        </div>
        ...
        <div class="w-28 h-28 bg-gradient-to-br from-brand-50 to-brand-100 border-4 border-white rounded-2xl mx-auto mb-5 flex items-center justify-center text-brand-600 text-4xl font-extrabold shadow-lg shadow-brand-500/10">
            <?= strtoupper(mb_substr($data['full_name'], 0, 1)) ?>
        </div>
        ...
        <div class="bg-surface-50 p-4 rounded-2xl inline-block my-5 border border-surface-200/40">
            ...
            <p class="text-[9px] text-surface-800/25 mt-2 font-mono uppercase tracking-widest">Scan to verify</p>
        </div>

        <div class="text-left bg-surface-50 rounded-2xl p-5 space-y-3 border border-surface-200/40">
            <div class="flex justify-between items-center">
                <span class="text-[10px] font-semibold text-surface-800/30 uppercase tracking-wider">Host</span>
                <span class="text-sm font-bold text-surface-800"><?= sanitize($data['person_to_visit']) ?></span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-[10px] font-semibold text-surface-800/30 uppercase tracking-wider">Purpose</span>
                <span class="text-sm font-bold text-surface-800"><?= sanitize($data['purpose_of_visit']) ?></span>
            </div>
            <div class="flex justify-between items-center border-t border-surface-200/40 pt-3">
                <span class="text-[10px] font-semibold text-surface-800/30 uppercase tracking-wider">Check-in</span>
                <span class="text-sm font-bold text-brand-600 tabular-nums"><?= date('h:i A', strtotime($data['check_in_time'])) ?></span>
            </div>
        </div>

        <p class="mt-5 text-[9px] text-surface-800/20 font-medium">Amen's Office Visitor Management System</p>
    </div>
</div>

<div class="mt-8 no-print flex gap-3">
    <button onclick="window.print()" class="bg-white text-brand-700 font-semibold px-7 py-3 rounded-2xl shadow-lg shadow-surface-900/5 border border-surface-200/60 hover:bg-surface-50 transition-all active:scale-95 flex items-center gap-2 text-sm">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
        Print badge
    </button>
    <button onclick="window.close()" class="bg-surface-900 text-white font-semibold px-7 py-3 rounded-2xl shadow-lg hover:bg-surface-800 transition-all active:scale-95 text-sm">Close</button>
</div>
</body>
</html>
